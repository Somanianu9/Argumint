...
contract Argumint {
    ...
    ); // may need the current team or extract it from the user
    ...
    function setAMT(address _amt) external onlyOwner {
    ...
    function createDebate(
        ...
        if (startDelay < 1 || duration < 1) {
            revert invalidDebateData(
                "Start delay and duration must be at least 1 second"
            );
        }
        debateCount += 1;
    ...
    function finishDebate(uint256 id) external {
        ...
        for (uint i = 0; i < winners.length; ++i) {
            ...
            amt.mint(user, 3); // 3 tokens to winners
        }

        // 7) Reward losers with 1 token
        address[] storage losers = (team1Score > team2Score)
            ? d.team2
            : d.team1;
        for (uint i = 0; i < losers.length; ++i) {
            amt.mint(losers[i], 1); // 1 token to losers
        }
    }
